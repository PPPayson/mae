...
def get_3d_sincos_pos_embed(embed_dim, grid_sizes, cls_token=False):
    ...
    grid_d = np.arange(grid_sizes[2], dtype=np.float32)
...
def get_3d_sincos_pos_embed_from_grid(embed_dim, grid):
    if embed_dim % 3 == 0:
        embed_dims = [embed_dim//3]*3
    else:
        embed_dims = [embed_dim//3, embed_dim//3, embed_dim//3+embed_dim%3]
    emb_h = get_1d_sincos_pos_embed_from_grid(embed_dims[0], grid[0])  # (H*W, D/2)
    emb_w = get_1d_sincos_pos_embed_from_grid(embed_dims[1], grid[1])  # (H*W, D/2)
    emb_d = get_1d_sincos_pos_embed_from_grid(embed_dims[2], grid[2])  # (H*W, D/2)
