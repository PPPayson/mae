...
class MaskedAutoencoderTIMM(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.decoder_modules = nn.Sequential(self.decoder_embed, self.decoder_blocks, self.decoder_pred)
        # --------------------------------------------------------------------------
    ...
    def initialize_weights(self):
        ...
        self.decoder_modules.apply(self._init_weights)
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        x = self.encoder.model._pos_embed(x)
        x = self.encoder.model.norm_pre(x)
        cls_token = x[:, 0, :]
        x = x[:, 1:, :]
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75, f2d=False, pool=False):
        if f2d:
            output, _, _ = self.forward_encoder(imgs, mask_ratio=0)
            return output
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(imgs, pred, mask)
...
def mae_vit_small_patch16_dec192d4(**kwargs):
    model = MaskedAutoencoderViT(
...
def mae_vit_small_patch16_timm_dec192d4(**kwargs):
    model = MaskedAutoencoderTIMM(
        patch_size=16, embed_dim=384, depth=12, num_heads=6,
        decoder_embed_dim=192, decoder_depth=4, decoder_num_heads=3,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model 
...
mae_vit_small_patch16 = mae_vit_small_patch16_dec192d4 # decoder: 192 dim, 4 blocks
mae_vit_small_patch16_timm = mae_vit_small_patch16_timm_dec192d4
