...
def main(args):
    ...
    patch_size = model.student_encoder.patch_embed.patch_size
    ...
    if args.distributed:
        if has_batchnorms(model.student):
            model.student = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.student)
            model.teacher = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.teacher)
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=False)
        model_without_ddp = model.module

    model_opt = model_without_ddp.student
    ...
    momentum_schedule = cosine_scheduler(args.momentum_teacher, 1, args.epochs, num_training_steps_per_epoch)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = dino_train_one_epoch(
        ...
        val_stats = dino_eval_one_epoch(
                        model, model_without_ddp,data_loader_val, optimizer,
                        device, epoch,
                        log_writer=log_writer,
